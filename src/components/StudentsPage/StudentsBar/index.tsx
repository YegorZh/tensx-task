...
import exportStudentsData from "../../../exportToCSV";
import { setCurrentPage } from "../../../redux/pageSlice";

const StudentsBar: React.FC = () => {
    const searchValue = useAppSelector((state) => state.search.value);
    const data = useAppSelector((state) => state.studentsData.data);
        ...
        <div className="StudentsBar">
            ...
            <div className="StudentsBar-search">
                <div className="StudentsBar-search-wrapper">
                    <input
                        onChange={(event) => {
                            dispatcher(setSearchValue(event.target.value));
                        }}
                        value={searchValue}
                        type="textfield"
                        className="StudentsBar-search-input"
                        placeholder="Enter Student Name, Parent or ID here" />
                    <button className="StudentsBar-search-button" onClick={() => {
                        dispatcher(setCurrentPage(1));
                        dispatcher(stampSearch());
                    }}>
            ...
            <div className="StudentsBar-export">
                <button className="StudentsBar-export-button" onClick={() => { if (data) exportStudentsData(data) }}>
